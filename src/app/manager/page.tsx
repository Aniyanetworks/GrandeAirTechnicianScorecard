...
export default function ManagerPage() {
  ...
  useEffect(() => {
    ...
    const avgScore = cards.length ? Math.round(cards.reduce((s, c) => s + c.card.overallScore, 0) / cards.length) : 0;
    ...
    setTeamStats({ green, yellow, red, avgScore, teamRevenue, teamJobs, teamFCC: teamFCC, teamMAS: teamMASr, teamCSAT, teamCallbacks: teamCBr });
    ...
    const kpis: MgrKPI[] = [
      ...
      { name: "Coaching Sessions",    value: coaching, score: redCount === 0 ? 100 : ts(coaching, 0, Math.max(redCount, 1) * 2), weight: 5, color: getScoreColor(redCount === 0 ? 100 : ts(coaching, 0, redCount * 2)), target: `${Math.max(redCount, 1) * 2}+`, actual: String(coaching) },
    ];
